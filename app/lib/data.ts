...
import { TextAnalysisResult, IpCheckResult, VirusScanResult } from './definitions';

const sql = postgres(process.env.POSTGRES_URL!, { ssl: 'require' });

/**
 * Пример: Получение количества анализов текста
 */
...
/**
 * Пример: Получение количества проверок IP
 */
...
/**
 * Пример: Получение количества сканирований вирусов
 */
...
/**
 * Получение последнего анализа текста
 */
...
export async function fetchLatestIpChecks(limit: number = 5): Promise<IpCheckResult[]> {
  try {
    const data = await sql<IpCheckResult[]>`
      SELECT id, malicious, abuse_confidence_score as "abuseConfidenceScore", country
      FROM ip_checks
